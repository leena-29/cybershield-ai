...
class PhishingHeuristics:
    ...
    SUSPICIOUS_TLDS = {
        'xyz', 'tk', 'ml', 'ga', 'cf', 'gq', 'top', 'work', 'buzz', 'info', 'fo'
    }
    ...
    def detect_brand_impersonation(hostname):
        reasons = []
        normalized = PhishingHeuristics.normalize_domain(hostname)
        base_domain = PhishingHeuristics.get_base_domain(normalized)
    ...
    @staticmethod
    def has_suspicious_random_domain_pattern(hostname):
        normalized = PhishingHeuristics.normalize_domain(hostname)
        first_label = normalized.split('.')[0] if normalized else ''

        has_letters = any(ch.isalpha() for ch in first_label)
        has_numbers = any(ch.isdigit() for ch in first_label)

        if not (has_letters and has_numbers):
            return False

        # Patterns like new6, x7hd, ab12cd, x1z2q3
        mixed_pattern = re.search(r'[a-z]+\d+[a-z0-9]*|[a-z]*\d+[a-z]+', first_label)
        return bool(mixed_pattern) and len(first_label) >= 4

    @staticmethod
    def has_suspicious_tld(hostname):
        normalized = PhishingHeuristics.normalize_domain(hostname)
        tld = normalized.split('.')[-1] if '.' in normalized else ''
        return tld in PhishingHeuristics.SUSPICIOUS_TLDS

    @staticmethod
    def has_tracking_params(query):
        lowered = (query or '').lower()
        return 'utm=' in lowered or 'ref=' in lowered or 'redirect=' in lowered
    ...
    def analyze(url):
        ...
        base_domain = PhishingHeuristics.get_base_domain(hostname)
        ...
        if PhishingHeuristics.has_suspicious_random_domain_pattern(hostname):
            reasons.append('Suspicious domain pattern detected (random letters and numbers)')
            score += 20

        if base_domain and base_domain not in PhishingHeuristics.TRUSTED_DOMAINS:
            reasons.append('Unknown or untrusted domain')
            score += 10

        if PhishingHeuristics.has_suspicious_tld(hostname):
        ...
        if subdomain_count > 2:
            reasons.append('More than two subdomains before base domain')
            score += 15

        if PhishingHeuristics.has_tracking_params(parsed.query):
            reasons.append('Tracking parameters detected (utm/ref/redirect)')
            score += 10
