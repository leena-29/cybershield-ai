...
from ai_models.phishing_detector import phishing_model, PhishingHeuristics
...
@scanner_bp.route('/phishing-detect', methods=['POST'])
@token_required
def phishing_detect():
    """Dedicated phishing detection endpoint"""
    try:
        data = request.get_json() or {}
        url = (data.get('url') or '').strip()
        ...
        logger.error(f"Phishing detect error: {e}")
        return jsonify({'error': str(e)}), 500
