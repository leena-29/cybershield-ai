...
class IPIntelligence:
    ...
    PRIMARY_SOURCE = 'ipapi.co'
    FALLBACK_SOURCE = 'ipinfo.io'
    ...
    @staticmethod
    def _parse_ipapi_data(ip, data):
        if data.get('error'):
            raise ValueError(data.get('reason') or 'Unable to fetch IP data')

        return {
            'source': IPIntelligence.PRIMARY_SOURCE,
            'ip': IPIntelligence._normalize_text(data.get('ip')) or ip,
    ...
    @staticmethod
    def _parse_ipinfo_data(ip, data):
        if data.get('bogon'):
            raise ValueError('Unable to fetch IP data')

        country_code = IPIntelligence._normalize_text(data.get('country')) or 'Unknown'
        region = IPIntelligence._normalize_text(data.get('region')) or 'Unknown'
        city = IPIntelligence._normalize_text(data.get('city')) or 'Unknown'
        isp = IPIntelligence._normalize_text(data.get('org')) or 'Unknown'
        loc = IPIntelligence._normalize_text(data.get('loc'))
        ...
        return {
            'source': IPIntelligence.FALLBACK_SOURCE,
            'ip': IPIntelligence._normalize_text(data.get('ip')) or ip,
    ...
    def _fetch_public_ip_data(ip):
        attempts = [
            ('primary', f'https://ipapi.co/{ip}/json/', IPIntelligence._parse_ipapi_data),
            ('fallback', f'https://ipinfo.io/{ip}/json', IPIntelligence._parse_ipinfo_data)
        ]

        errors = []
        for _, url, parser in attempts:
            try:
                data = IPIntelligence._request_json(url, timeout=10)
                parsed = parser(ip, data)
                if parsed.get('country') == 'Unknown' and parsed.get('city') == 'Unknown' and parsed.get('isp') == 'Unknown':
                    raise ValueError('Unable to fetch IP data')
                return parsed
            except Exception as exc:
                errors.append(str(exc))

        raise requests.RequestException('; '.join(errors) or 'Unable to fetch IP data')
    ...
    def check_reputation(ip, info):
        ...
        missing_location_parts = [part for part in (country, region, city) if part.lower() in IPIntelligence.UNKNOWN_REGION_VALUES]
