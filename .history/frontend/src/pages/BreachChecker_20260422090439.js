...
const BreachChecker = () => {
  ...
  const handleCheck = async (e) => {
    ...
    setCopied(false);
    try {
      const response = await scanAPI.breachCheck(email.trim());
      ...
      >
        <form onSubmit={handleCheck} className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <Button variant="primary" size="lg" loading={loading} type="submit" className="w-full" disabled={loading || !email.trim()}>
              Check Breaches
            </Button>
            <Button
              variant="outline"
