...
export const scanAPI = {
  ...
  analyzeDomain: (domain) => api.post('/domain/analyze', { domain }, { timeout: 15000 }),
  scanDomain: (domain) => api.post('/scan/domain', { domain }),
  breachCheck: (email) => api.post('/breach-check', { email }, { timeout: 12000 }),
  scanBreach: (email) => api.post('/scan/breach', { email }),
  scanIP: (ip) => api.post('/scan/ip', { ip }, { timeout: 15000 }),
