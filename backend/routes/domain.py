...
def _normalize_domain_variants(domain):
    variants = {''}
    for char in domain:
        if char == '1':
            variants = {variant + replacement for variant in variants for replacement in ('l', 'i')}
        else:
            replacement = LOOKALIKE_SUBSTITUTIONS.get(char, char)
            variants = {variant + replacement for variant in variants}
    return variants


def _is_lookalike_domain(domain):
    if _is_official_or_subdomain(domain):
        return False

    for normalized in _normalize_domain_variants(domain):
...
def _calculate_score(domain, age_days):
    ...
    lookalike_domain = _is_lookalike_domain(domain)
    ...
    if lookalike_domain:
        score += 50
        indicators.append('Possible lookalike (typosquatting) domain')
    ...
    if lookalike_domain and score >= 50:
        risk_level = 'High'
    elif score >= 61:
        risk_level = 'High'
    elif score >= 31:
